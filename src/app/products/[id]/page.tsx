...
export default function ProductDetailPage() {
  ...
  // ===== NEW: gallery từ product.images, fallback thumbnail =====
      ...
      <section className="grid gap-6 rounded-2xl border border-slate-200 bg-white p-4 md:p-6 lg:grid-cols-[1.05fr_1.6fr]">
        ...
        <div className="flex gap-4">
          ...
          <div className="hidden w-20 shrink-0 flex-col gap-2 sm:flex">
            {gallery.map((img, i) => (
              <button
                ...
                className={`relative overflow-hidden rounded-md border transition hover:border-slate-900 cursor-pointer ${
                  i === activeIndex
                    ? "border-slate-900 ring-slate-200"
                    : "border-slate-200"
                }`}
              >
          ...
          <div className="flex flex-1 items-start justify-center">
            <div className="relative aspect-square w-full max-w-[480px] rounded-xl border border-slate-200 bg-white overflow-hidden">
              <img
                ref={mainImgRef}
                src={activeImage}
                alt={product.name}
                className="absolute inset-0 object-contain p-6 w-full h-full"
              />
            </div>
          </div>
        </div>
                      ...
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star
                          ...
                          className={`h-3.5 w-3.5 ${
                            i < r.rating ? "fill-amber-400" : "text-slate-300"
                          }`}
                        />
                      ))}
                    </div>
