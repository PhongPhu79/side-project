...
export function ProductCard({ product }: { product: Product }) {
    ...
    const handleAddToCart = () => {
        ...
        // 2) Fly-to-cart animation (Shopee style)
    ...
    return (
        <Card className="flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-200 hover:-translate-y-1 hover:shadow-md">
            {/* Image + badge */}
            <div className="relative">
                <Link href={`/products/${product.id}`} className="block">
                    <div
                        ref={imageRef}
                        className="relative flex aspect-4/3 items-center justify-center bg-slate-50"
                    >
                        <img
                        ...
                        <span className="absolute inset-0 hidden items-center justify-center bg-slate-200 px-3 text-center text-xs text-slate-500">
                            {product.name}
                        </span>
                    </div>
                </Link>
            ...
            <div className="flex flex-1 flex-col gap-1 px-4 py-3">
                ...
                <Link
                    ...
                    className="line-clamp-2 text-sm font-medium text-slate-900 hover:text-slate-700"
                >
                    {product.name}
                </Link>
