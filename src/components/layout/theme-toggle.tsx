...
export function ThemeToggle() {
  ...
  // Lần render đầu có thể chưa có resolvedTheme → tránh nháy
  if (!resolvedTheme) {
    return (
      <button
        type="button"
        aria-label="Toggle theme"
        className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-700 text-xs"
        disabled
      >
        …
      </button>
